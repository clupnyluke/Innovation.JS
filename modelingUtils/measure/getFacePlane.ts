...
const getPlaneOfFace = (oc: OpenCascadeInstance, face: TopoDS_Face, normalized = true) => {
  ...
  if (surface.GetType() !== oc.GeomAbs_SurfaceType.GeomAbs_Plane) throw new Error("Face is not a plane")
  ...
  oc.BOPTools_AlgoTools3D.GetNormalToSurface(surface.Surface().Surface(), samplePoint.X(), samplePoint.Y(), planeNormal)

  let plane = [planeNormal.X(), planeNormal.Y(), planeNormal.Z(), pointOnPlane.XYZ().Dot(planeNormal.XYZ())]
  if (normalized) {
    if (plane[3] < 0) {
